...
(function(jQuery, window, document, undefined) {
    ...
    jQuery.fn.audioPlayer = function(params) {
        ...
        this.each(function() {
            ...
            jQuery(this).replaceWith(thePlayer);
        });
        return this;
    };
...
{	
	...
	{
		this.jQuery(window).on('scroll.parallax.begin', this._beginScroll.bind(this));

		this.jQuery(window).on('scroll.parallax.debounce', debounce(function ()
		{
		...
		this.jQuery(window).on('resize.parallax', debounce(this.refresh.bind(this), 30));
	};
	...
	Parallax.prototype._beginScroll = function ()
	{
		if (!this.scrolling)
		{
			this._go();
			this.scrolling = true;
			this.jQuery(window).off('scroll.parallax.begin');			
		}
	};
	...
	{	
		...
		this.jQuery(el).each(function (index, el)
		{	
			var $this            = jQuery(this),
			...
			self.speeds[index]    = jQuery(this).attr('data-speed');
		});
	};
	...
	{
        var viewportTop = this.jQuery(window).scrollTop();
        ...
        this.lastScrollTop = viewportTop;
 		...
 		for (var i = 0, l = this.jQuery(el).length; i < l; i++)
 		{
 			var el = this.jQuery(el)[i];
			...
			jQuery(el).css('transform','translate3d(0, ' + this.currentTransforms[i] +'px,0)'); 			
 		}
	};
